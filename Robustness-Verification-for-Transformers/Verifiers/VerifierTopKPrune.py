...
def sample_correct_samples(args, data, target):
    ...
    attempts = 0
    while len(examples) < num_samples and attempts < num_samples * 5:
        attempts += 1
...
# --- Stable Softmax Implementation with Masking ---
...
def softmax_with_mask(zonotope: Zonotope, 
    ...
    """
    Applies Softmax with an optional binary mask for pruning.
    """
    ...
    # 2. Apply Mask: m * e^x
    # Zeros out the exponentiated term for pruned tokens
    if mask is not None:
        zonotope_exp = zonotope_exp.multiply(mask)

    # 3. Compute Denominator: Sum(m * e^x)
    # Summing the ALREADY MASKED terms ensures the probability sums to 1.0 relative to kept tokens
...
class VerifierTopKPrune(Verifier):
    ...
    def _run_single_model_bounds(self, image: torch.Tensor, eps: float, is_pruned: bool) -> Optional[Zonotope]:
        ...
        try:
            ...
            if self.debug: print(f"Error in single model run (Pruned={is_pruned}): {e}")
            bounds = None
        finally:
    ...
    def run(self, data) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        # Determine effective K for logging
    ...
    def _bound_attention(self, bounds_input: Zonotope, attn, layer_num=-1) -> Tuple[Zonotope, Zonotope, Zonotope, Zonotope]:
        ...
        if self.token_pruning_enabled and layer_num >= self.prune_layer_idx:
            ...
            # Prioritize 'tokens_to_prune' (Bottom-X) logic if set
            if self.tokens_to_prune > 0:
            ...
            # To be safely pruned, a token's UPPER bound must be smaller than the 
            # LOWER bound of the k-th best token.
            
            # Find the k-th largest LOWER bound value along the last dimension
            # .values shape: [batch, heads, tokens, k_keep]
            topk_values = torch.topk(l_scores, k_keep, dim=-1, largest=True, sorted=True).values
            
            # The cutoff is the smallest value in the top-K set (the "worst" winner)
            cutoff_threshold = topk_values[..., -1].unsqueeze(-1)
            
            # 4. Generate Mask
            # Keep (1.0) if UpperBound >= Threshold (Meaning it *might* be a winner)
            # Prune (0.0) if UpperBound < Threshold (Meaning it is definitely a loser)
