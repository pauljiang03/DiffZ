...
def analyze_all_results(results_diff: List[Dict[str, Any]],
    ...
    print("\n" + "="*100)
    print(f"DETAILED SAMPLE ANALYSIS ({valid_samples} SAMPLES) - COUPLED EXECUTION")
    ...
    print("Difference is computed SYMBOLICALLY: Diff = Z(P) - Z(P')")
    print("="*100)
    ...
    for i in range(valid_samples):
        ...
        print(f"   {'Class':<5} | {'P Low':<10} | {'P Up':<10} | {'P\' Low':<10} | {'P\' Up':<10} | {'SymDiff L':<10} | {'SymDiff U':<10}")
        print("-" * 95)
    ...
    print("\n" + "="*80)
    print("AGGREGATE METRICS (COUPLED)")
    print("="*80)
    
    print("1. Average Symbolic Differential Bounds (Real Class):")
    print(f"   Lower Bound (Min Diff): {avg_L_real:.5f}")
    print(f"   Upper Bound (Max Diff): {avg_U_real:.5f}")
